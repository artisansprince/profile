...
const featuredProjectIds = [1, 2, 3]; // ID project unggulan
const featuredProjectCardsContainer = document.getElementById('featured-projects');

if (featuredProjectCardsContainer) {
    // Menampilkan hanya proyek unggulan berdasarkan ID yang ada dalam featuredProjectIds
    featuredProjectIds.forEach(id => {
        const project = projects.find(p => p.id === id);
        
        if (project) {
            const card = document.createElement('div');
            card.className = 'card text-black rounded-lg m-2 w-80 text-start';

            // Menghitung diskon
            const discount = ((project.originalPrice - project.price) / project.originalPrice) * 100;

            // Menambahkan konten card sesuai dengan data project
            card.innerHTML = `
            <a href="#" onclick="viewDetail(${project.id})">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex justify-between text-md font-semibold mt-2">
            ...
            // Menambahkan card ke dalam container featured-projects
            featuredProjectCardsContainer.appendChild(card);
        }
    });
}
...
// Menampilkan seluruh project cards di halaman index
const allProjectCardsContainer = document.getElementById('project-cards');
if (allProjectCardsContainer) {
    projects.forEach(project => {
        ...
        allProjectCardsContainer.appendChild(card);
    });
}
...
if (projectId) {
    ...
    if (project) {
        ...
        project.images.forEach(image => {
            ...
            imgElement.className = 'w-full rounded border-2 border-zinc-200';
    ...
    if (currentProjectIndex !== -1) {
        ...
        if (relatedProjects.length > 0) {
            relatedProjects.forEach(project => {
                ...
                projectElement.className = 'card border-2 border-zinc-800 text-black rounded-lg p-4';
                ...
                projectElement.innerHTML = `
                    ...
                    <button class="mt-4 py-2 px-4 bg-black text-white w-full rounded" onclick="viewDetail(${project.id})">View Detail</button>
                `;
